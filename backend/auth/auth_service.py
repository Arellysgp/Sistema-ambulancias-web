...
ROLES_VALIDOS = ['operador', 'conductor', 'admin']
...
def registrar_usuario(nombre, email, password, rol='operador'):
    ...
    if rol not in ROLES_VALIDOS:
        return None, "Rol inválido. Debe ser: operador, conductor o admin"
    if User.query.filter_by(email=email).first():
        return None, "El email ya está registrado"

    hash_pw = generate_password_hash(password)
    nuevo = User(nombre=nombre.strip(), email=email.lower(),
                 password_hash=hash_pw, rol=rol)
